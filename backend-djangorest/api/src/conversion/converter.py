...
class CurrencyConversionService:
    ...
    def make_conversion(currency_from: str, currency_to: str, ammount=1) -> float:
        code_from = min(currency_from, currency_to)
        code_to = max(currency_from, currency_to)
        ...
        Conversion.objects.create(
            currency_from=Currency.objects.get(code=code_from),
            currency_to=Currency.objects.get(code=code_to),
    ...
    def delete_conversions():
        ...
        currency_conversions = Conversion.objects.filter(created_at__lt=limit_date)
